...
let selectedClient = (DATA && DATA.clients && DATA.clients.length > 0) ? [...DATA.clients].sort((a,b)=>b.score-a.score)[0].client : "";
...
function currentPeriodLabel(){ return `${dateRange.start} â†’ ${dateRange.end}`; }
...
function renderCurrencyController(){
  ...
  return `<div class="currency-controller">
    ...
    ${hasMixedCurrencies()?`<span class="currency-note">Monedas detectadas: ${list.join(", ")} Â· totales normalizados</span>`:''}
  </div>`;
}
...
const cBy = n => DATA.clients.find(c=>c.client===n) || DATA.clients[0];
...
function renderAdDrilldown(){
  ...
  return `<div class="drilldown-panel">
    <div class="kpi"><div><h3>Leads asociados al anuncio</h3><p class="small">${a.ad_name_norm||'Sin nombre'} Â· ${a.client}</p></div><button class="ads-filter-btn" onclick="closeAdDrilldown()">Cerrar</button></div>
...
function buildFunnelIntelligence(clients){
  ...
  return clients.map(c=>{
    ...
    return {client:c.client,stages,health_matrix,leakages,biggest_leak:leakages.slice().sort((a,b)=>b.lost-a.lost)[0],bottleneck,benchmark_appointments:Math.round(c.leads*(DATA.benchmarks.avg_appointment_rate||0)),opportunity_appointments:Math.max(0,Math.round(c.leads*(DATA.benchmarks.avg_appointment_rate||0))-c.appointments),economics:[{stage:"Meta Results",value:c.meta_results,cost_per:pct2(c.spend,c.meta_results)},{stage:"CRM Leads",value:c.leads,cost_per:pct2(c.spend,c.leads)},{stage:"CRM Movement",value:c.moved,cost_per:pct2(c.spend,c.moved)},{stage:"Appointments",value:c.appointments,cost_per:pct2(c.spend,c.appointments)}],note_crm_exceeds_meta:c.leads>c.meta_results&&c.meta_results>0};
  });
}
